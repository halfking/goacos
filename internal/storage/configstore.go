...
const configCols = "id, data_id, group_id, content, md5, gmt_create, gmt_modified, IFNULL(src_user,''), IFNULL(src_ip,''), IFNULL(app_name,''), tenant_id, IFNULL(c_desc,''), IFNULL(c_use,''), IFNULL(effect,''), IFNULL(type,''), IFNULL(c_schema,'')"
	...
	if created {
		res, err := s.DB.ExecContext(ctx,
			`INSERT INTO config_info (data_id, group_id, content, md5, gmt_create, gmt_modified, src_user, src_ip, app_name, tenant_id, c_desc, c_use, effect, type, c_schema)
			 VALUES (?,?,?,?,NOW(),NOW(),?,?,?,?,?,?,?,?,?)`,
			u.DataId, u.GroupId, u.Content, md5v, nullIfEmpty(u.SrcUser), nullIfEmpty(u.SrcIp), nullIfEmpty(u.AppName), u.TenantId,
...
func (s *Store) ListConfigs(ctx context.Context, q ListQuery) ([]ConfigInfo, int64, error) {
	...
	if q.DataId != "" {
		if q.Accurate {
			...
			where = append(where, "data_id LIKE CONCAT('%',?,'%')")
			args = append(args, q.DataId)
		}
	}
	if q.GroupId != "" {
		if q.Accurate {
			...
			where = append(where, "group_id LIKE CONCAT('%',?,'%')")
			args = append(args, q.GroupId)
		}
	}
	if q.AppName != "" {
	...
	var total int64
	if err := s.DB.QueryRowContext(ctx, "SELECT COUNT(*) FROM config_info"+join+" WHERE "+w, args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	if q.PageNo < 1 {
		q.PageNo = 1
	...
	rows, err := s.DB.QueryContext(ctx, "SELECT "+configCols+" FROM config_info"+join+" WHERE "+w+order, args...)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
const historyCols = "nid, id, data_id, group_id, IFNULL(app_name,''), content, IFNULL(md5,''), gmt_create, gmt_modified, IFNULL(src_user,''), IFNULL(src_ip,''), op_type, tenant_id"
...
func (s *Store) ListHistory(ctx context.Context, q HistoryListQuery) ([]HistoryInfo, int64, error) {
	...
	w := strings.Join(where, " AND ")
	var total int64
	if err := s.DB.QueryRowContext(ctx, "SELECT COUNT(*) FROM his_config_info WHERE "+w, args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	if q.PageNo < 1 {
		q.PageNo = 1
...
func (s *Store) GetHistory(ctx context.Context, nid int64) (*HistoryInfo, error) {
	h, err := scanHistory(s.DB.QueryRowContext(ctx,
