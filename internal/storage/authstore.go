...
func (s *Store) GetUser(ctx context.Context, username string) (*User, error) {
	var u User
	var enabled int
	err := s.DB.QueryRowContext(ctx, "SELECT username, password, enabled FROM users WHERE username=?", username).
...
func (s *Store) CreateUser(ctx context.Context, username, passwordHash string) error {
	_, err := s.DB.ExecContext(ctx, "INSERT INTO users (username, password, enabled) VALUES (?,?,1)", username, passwordHash)
	return err
}
...
func (s *Store) DeleteUser(ctx context.Context, username string) error {
	if _, err := s.DB.ExecContext(ctx, "DELETE FROM roles WHERE username=?", username); err != nil {
		return err
	}
...
func (s *Store) ListUsers(ctx context.Context, pageNo, pageSize int) ([]User, map[string][]string, int64, error) {
	...
	rows, err := s.DB.QueryContext(ctx, "SELECT username, password, enabled FROM users ORDER BY username LIMIT ? OFFSET ?",
	...
	rrows, err := s.DB.QueryContext(ctx, "SELECT username, role FROM roles")
	if err != nil {
...
func (s *Store) ListRolesByUser(ctx context.Context, username string) ([]string, error) {
	rows, err := s.DB.QueryContext(ctx, "SELECT role FROM roles WHERE username=?", username)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) AddRole(ctx context.Context, username, role string) error {
	_, err := s.DB.ExecContext(ctx, "INSERT IGNORE INTO roles (username, role) VALUES (?,?)", username, role)
	return err
}

// DeleteRole revokes a role from a user.
func (s *Store) DeleteRole(ctx context.Context, username, role string) error {
	_, err := s.DB.ExecContext(ctx, "DELETE FROM roles WHERE username=? AND role=?", username, role)
	return err
}
